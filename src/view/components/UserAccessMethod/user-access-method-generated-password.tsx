...
export function UserAccessMethodGeneratedPassword({
	...
	return (
        ...
        <Alert className="mt-8">
          <InfoIcon className="w-5 h-5 bg-info/10 dark:bg-info text-info-foreground ring-info" />
          <AlertTitle>Requisitos mínimos</AlertTitle>
          <AlertDescription>
            <ul className="space-y-1 text-sm">
							<li
								className={`flex items-center gap-2 ${value.length >= 8 ? "text-green-600" : "text-muted-foreground"}`}
							>
								{value.length >= 8 ? "✓" : "○"} Mínimo de 8 caracteres
							</li>
							<li
								className={`flex items-center gap-2 ${/[!@#$%^&*(),.?":{}|<>]/.test(value) ? "text-green-600" : "text-muted-foreground"}`}
							>
								{/[!@#$%^&*(),.?":{}|<>]/.test(value) ? "✓" : "○"} Pelo menos 1
								caractere especial
							</li>
							<li
								className={`flex items-center gap-2 ${/\d/.test(value) ? "text-green-600" : "text-muted-foreground"}`}
							>
